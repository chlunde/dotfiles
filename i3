...
bindsym XF86AudioRaiseVolume exec --no-startup-id "pactl set-sink-volume @DEFAULT_SINK@ +5%; pactl get-sink-volume @DEFAULT_SINK@ | grep -qE '[0-9]{3}%' && pactl set-sink-volume @DEFAULT_SINK@ 100%; $refresh_i3status"
...
bindsym $mod+t [class="Gnome-terminal"] focus
bindsym $mod+c [class="Google-chrome"] focus
bindsym $mod+u [class="Emacs"] focus
bindsym $mod+Tab exec rofi -show window --auto-select
bindsym Mod1+Tab exec rofi -show window --auto-select
...
bar {
        status_command ~/go/bin/status-bar
}
